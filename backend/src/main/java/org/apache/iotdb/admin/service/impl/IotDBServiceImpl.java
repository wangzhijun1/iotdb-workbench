...
public class IotDBServiceImpl implements IotDBService {
    ...
    try {
      ...
      if (batchSize > 0) {
        while (sessionDataSetWrapper.hasNext()) {
          ...
          List<org.apache.tsfile.read.common.Field> fields = rowRecord.getFields();
          ...
          if (keyword != null || "".equals(keyword)) {
            ...
            for (int i = 0; i < fields.size(); i++) {
              java.lang.reflect.Field field =
                  MeasurementDTO.class.getDeclaredField(columnNames.get(i).replaceAll(" ", "").toLowerCase());
  ...
  private MetricsChartDataVO getInterfaceTime(
    ...
    //            RowRecord rowRecord = sessionDataSetWrapper.next();
    //            long timestamp = rowRecord.getTimestamp();
    //            List<Field> fields1 = rowRecord.getFields();
  ...
  private String executeQueryOneLine(SessionPool sessionPool, String sql, String queryField)
    ...
    try {
      ...
      int index = -1;
      for (int i = 0; i < columnNames.size(); i++) {
        if (queryField.equalsIgnoreCase(columnNames.get(i))) {
          index = i;
        }
      }
      if (index == -1) {
      ...
      if (batchSize > 0) {
        while (sessionDataSetWrapper.hasNext()) {
          ...
          if (count >= pageSize * (pageNum - 1) + 1 && count <= pageSize * pageNum) {
            ...
            List<String> columnNames = sessionDataSetWrapper.getColumnNames();
            for (int i = 0; i < fields.size(); i++) {
              java.lang.reflect.Field field = clazz.getDeclaredField(columnNames.get(i).replaceAll(" ", ""));
              field.setAccessible(true);
              field.set(t, fields.get(i).toString());
            }
            results.add(t);
    ...
    for (String s : compression) {
      switch (s) {
        ...
        case "ZSTD":
          ...
          break;
        case "LZMA2":
          list.add(CompressionType.LZMA2);
          break;
        default:
          throw new BaseException(
